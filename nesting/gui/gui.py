...
class MainWindow(Ui_MainWindow, QMainWindow):
    ...
    def drawShapeInWorkspace(self):  # TODO: Delete
        if not self.api.selected_shape_name:
            return
    ...
    def workspaceExport(self):
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(
            self,
            "Where to save current worksapce...",
            "Workspace.json",
        ...
        if self.mode == Mode.drawing or self.mode == Mode.subtracting:
            ...
            if not self.first_point:
                ...
                if (
                    ...
                    and abs(y - self.first_point[1]) < 50
                    and len(self.drawn_shape) > 2
                ):
    ...
    def workspaceMouseClicked(self, event):
        ...
        if self.mode == Mode.drawing or self.mode == Mode.subtracting:
            ...
            if event.button == 1:  # left mouse button
                if self.hole_to_remove:
                    self.api.optimiser.removeHole(self.hole_to_remove)
                self.stopDeleting()
                self.startDeleting()
            elif event.button == 3:  # right mouse button
                self.stopDeleting()
