...
from nesting.optimiser import spaceoptimiser
from nesting.optimiser.localsearch import LocalSearch
...
class Api:
    ...
    def placeSelectedShape(self):
        ...
        if self.getSelectedShapeCount() == 0:
            return None  # TODO: error code
        ...
        if self.settings.use_nfp:
            ...
            if not self.optimiser.begin():
                return False  # TODO: error code

        if self.settings.local_optimisation:
            g_search = LocalSearch(
                self.optimiser.shape,
    ...
    def placeAllSelectedShapes(self):
        ...
        shapes_sorted = sorted(
            ...
            reverse=True,
        )  # names of shapes sorted by area, descending
        for shape in shapes_sorted:
            ...
            for _ in range(self.shape_dict[shape]["count"]):
                ...
                print(
                    self.selected_shape_name,
                    "placed (",
                    self.num_placed_shapes,
                    "/",
                    self.num_shapes_to_place,
                    ")",
                )
    ...
    def constructShapeList(self, folder=None):
        ...
        if not folder:
            folder = self.settings.input_path
        else:
            self.settings.input_path = folder
    ...
    def getShapesFromGcode(self, file, axes=None):
        ...
        try:
            ...
            self.logger.log(
                f"{os.path.basename(file)} is not a valid NC program",
                self.logger.logLevel.WARNING,
                self.logger.logType.PARSER,
            )
            return []
