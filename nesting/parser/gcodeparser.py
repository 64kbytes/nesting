...
def interpolateArc(start, end, center, clockwise=True, maxlength=20, maxangle=0.4):
    """Returns a list of points along the specified arc from start to end, ommiting the endpoint"""
            ...
            if code >= 90:  # set absolute or incremental move
                ...
                if code == 1:  # straigh line (expexting X#.# Y#.#)
                    ...
                    # TODO: add exception checking
                        ...
                        self._split_coordinates(next(inst_iterator), "X")[1],
                        self._split_coordinates(next(inst_iterator), "Y")[1],
                    ]
                    center = [
                        self._split_coordinates(next(inst_iterator), "I")[1],
                        self._split_coordinates(next(inst_iterator), "J")[1],
                    ]
                    cw = True
                    if code == 3:
                        cw = False
