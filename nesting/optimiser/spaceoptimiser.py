...
class Optimiser:
    ...
    def getBoardHolesNFP(self):
        ...
        for hole in self.holes:
            ...
            if not npolys:
                self.logger.log(
                    f"couldn't compute NFP for {hole.name}, falling back to circle",
                    self.logger.logLevel.ERROR,
                    self.log_type,
                )
    ...
    def addShapeAsHole(self, name="undefined"):
        ...
        newhole.origin = affinity.rotate(
            Point(-self.circle_center[0], -self.circle_center[1]),
            self.angle,
            origin=(0, 0),
        )
